...
public:
    ...
    float top = 1.0f - spriteRowHeight * 1;
    ...
    glBegin(GL_QUADS);
        ...
        glVertex2f(x, y);           // Верхний левый угол экрана
        glTexCoord2f(right, bottom);            // Нижняя правая текстурная координата
        glVertex2f(x + texWidth / spriteColumnCount, y);  // Верхний правый угол экрана
        glTexCoord2f(right, top);               // Верхняя правая текстурная координата
        glVertex2f(x + texWidth / spriteColumnCount, y + texHeight * spriteRowHeight); // Нижний правый угол экрана
        glTexCoord2f(left, top);                // Верхняя левая текстурная координата
        glVertex2f(x, y + texHeight * spriteRowHeight); // Нижний левый угол экрана
    glEnd();
