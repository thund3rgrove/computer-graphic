...
public:
    ...
    float acceleration = 0.1f; // Acceleration due to key presses
    ...
    void render() {
        // Determine the appropriate row index based on the character's state
        int spriteRowIndex = 0; // Default to the first row (walking)

        if (velocityX != 0 || velocityY != 0) {
            // If the character is moving, render the walking animation (row 1)
            spriteRowIndex = 1;
        } else {
            // If the character is idle, occasionally render the rare idle animation (row 3)
            // You can implement the logic for the rare idle animation based on a timer or random chance
            // For simplicity, I'll assume a random chance of 1 in 300 frames
            if ((rand() % 300) == 0) {
                spriteRowIndex = 2;
            } else {
                // Otherwise, render the regular idle animation (row 2)
                spriteRowIndex = 1;
            }
        }
        ...
        float left = spriteColumnWidth * 2; // Assuming the elementIndex is always 2 for the character
        ...
        glBegin(GL_QUADS);
            ...
            glVertex2f(x + texWidth / spriteColumnCount, y);
            glTexCoord2f(right, top);
            glVertex2f(x + texWidth / spriteColumnCount, y + texHeight * spriteRowHeight);
            glTexCoord2f(left, top);
            glVertex2f(x, y + texHeight * spriteRowHeight);
        glEnd();
